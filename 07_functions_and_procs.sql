    ...
    BEGIN TRY
        ...
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM kg.Alert WHERE alert_id=@event_id)
                INSERT INTO kg.Alert (alert_id, raised_at, [rule], priority)
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@ticket_id, NULL, @status, @created_by, NULL);
        -- Upsert TicketAssets
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF NOT EXISTS (SELECT 1 FROM app.TicketAssets WHERE ticket_id=@ticket_id AND asset_id=@aid)
        ...
        BEGIN
            ...
            WHILE @i2 < @n2
            BEGIN
                DECLARE @aid2 INT = (SELECT value FROM OPENJSON(@asset_ids) WITH (value INT '$') WHERE [key]=CAST(@i2 AS NVARCHAR));
