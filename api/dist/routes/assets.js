...
function getRequestFromContext(request) {
    ...
    if (sqlConn && typeof sqlConn.request === 'function')
        return sqlConn.request();
    return null;
}
async function registerAssetRoutes(fastify) {
    const uploadDir = config_1.cfg.uploadDir || '/data/attachments';
    try {
        await fs_1.default.promises.mkdir(uploadDir, { recursive: true });
    }
    catch (e) {
        fastify.log.warn('Could not ensure uploadDir', e);
    }
    // Create or update asset
    fastify.post('/assets', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        try {
            ...
            const out = await (0, sql_1.withRls)(userId, async (conn) => {
    ...
    fastify.get('/assets/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        try {
            ...
            let res;
            if (localReq)
                res = await localReq.input('id', id).query(sql);
            else
                res = await (0, sql_1.withRls)(userId, async (conn) => conn.request().input('id', id).query(sql));
            ...
            let imgsRes;
            if (localReq)
                imgsRes = await localReq.input('id', id).query(imgsSql);
            else
                imgsRes = await (0, sql_1.withRls)(userId, async (conn) => conn.request().input('id', id).query(imgsSql));
    ...
    fastify.post('/assets/:id/images', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const id = Number(request.params.id);
        ...
        const filename = path_1.default.basename(mp.filename || `asset-${id}.bin`).replace(/[^a-zA-Z0-9._-]/g, '_');
        const tmp = path_1.default.join(uploadDir, `.tmp-${Date.now()}-${filename}`);
        const final = path_1.default.join(uploadDir, `${Date.now()}-${filename}`);
        const hash = crypto_1.default.createHash('sha256');
        let size = 0;
        try {
            const ws = fs_1.default.createWriteStream(tmp, { flags: 'wx' });
            mp.file.on('data', (chunk) => { hash.update(chunk); size += chunk.length; });
            await mp.toBuffer ? (async () => { const buf = await mp.toBuffer(); hash.update(buf); size = buf.length; await fs_1.default.promises.writeFile(tmp, buf); })() : new Promise((res, rej) => mp.file.pipe(ws).on('finish', res).on('error', rej));
            const digest = hash.digest();
            await fs_1.default.promises.rename(tmp, final);
            ...
            const localReq = getRequestFromContext(request);
            if (!localReq)
                return reply.code(500).send({ error: 'No request SQL context for transactional image insert' });
            const req = localReq;
            ...
            try {
                if (await fs_1.default.promises.stat(tmp).then(() => true).catch(() => false))
    ...
    fastify.post('/assets/:id/maintenance', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const id = Number(request.params.id);
        const body = request.body || {};
        const userId = Number(request.user?.sub);
        if (!userId)
        ...
        try {
            ...
            if (!localReq)
                return reply.code(500).send({ error: 'No request-bound SQL connection' });
            const req = localReq;
            req.input('asset_id', id);
