...
async function registerKnowledgeRoutes(fastify) {
    ...
    fastify.post('/knowledge/articles', async (request, reply) => {
        const schema = zod_1.z.object({
            ...
            solution_steps: zod_1.z.array(zod_1.z.string()).optional(),
            resolution_time_mins: zod_1.z.number().optional(),
    ...
    fastify.get('/knowledge/recommendations/:ticketId', async (request, reply) => {
    ...
    fastify.post('/knowledge/references', async (request, reply) => {
        const schema = zod_1.z.object({
            article_id: zod_1.z.number(),
            ticket_id: zod_1.z.number(),
            was_helpful: zod_1.z.boolean(),
            resolution_time_mins: zod_1.z.number().optional()
        });
        const body = schema.safeParse(request.body);
        if (!body.success) {
    ...
    fastify.get('/analytics/metrics/tickets', async (request, reply) => {
        try {
            const conn = await (0, sql_1.getSqlConnection)(request);
    ...
    fastify.get('/analytics/asset/:assetId/impact', async (request, reply) => {
        const params = zod_1.z.object({
