...
const rls_1 = require("../middleware/rls");
const sql_2 = require("../sql");
async function registerTicketRoutes(fastify) {
    ...
    fastify.get('/tickets/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        ...
        try {
            const localReq = (0, rls_1.getRequestFromContext)(request);
            let res;
            ...
            if (localReq) {
                ...
                res = await (0, sql_2.withRls)(userId, async (conn) => {
            ...
            const rowversionBuf = row.rowversion_bin;
            const rowversionBase64 = rowversionBuf
                ? rowversionBuf.toString('base64')
                : null;
            if (rowversionBase64)
                reply.header('ETag', `W/"${rowversionBase64}"`);
            const ticket = {
    ...
    fastify.post('/tickets', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const bodySchema = zod_1.z.object({
            ...
            substatus_code: zod_1.z.string().optional(),
            severity: zod_1.z.number().optional(),
            category_id: zod_1.z.number().optional(),
            summary: zod_1.z.string().min(1),
            description: zod_1.z.string().optional(),
            site_id: zod_1.z.number().optional(),
            assignee_user_id: zod_1.z.number().nullable().optional(),
            team_id: zod_1.z.number().nullable().optional(),
            vendor_id: zod_1.z.number().nullable().optional(),
            due_at: zod_1.z.string().nullable().optional(),
            sla_plan_id: zod_1.z.number().nullable().optional(),
            asset_ids: zod_1.z.array(zod_1.z.number()).optional(),
        ...
        try {
            ...
            if (localReq) {
                ...
                const out = res.recordset && res.recordset[0]
                    ? res.recordset[0].ticket_id
                    : null;
                return reply.code(201).send({ ticket_id: out });
            }
            const out = await (0, sql_2.withRls)(userId, async (conn) => {
                const r = await conn
                    .request()
    ...
    fastify.patch('/tickets/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
        const paramsSchema = zod_1.z.object({ id: zod_1.z.string() });
        const bodySchema = zod_1.z.object({ data: zod_1.z.any() });
        const params = paramsSchema.safeParse(request.params);
